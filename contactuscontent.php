		...
		if (!$captchaerror && !$emailerror && !$nameerror && !$titleerror && !$messageerror) {
			...
			if ($sitemgr_contactus_emails[0]) {
				foreach ($sitemgr_contactus_emails as $sitemgr_contactus_email) {
					system_mail($sitemgr_contactus_email, "[".EDIRECTORY_TITLE."] ".system_showText(LANG_NOTIFY_CONTACTUS)." - ".stripslashes(htmlspecialchars_decode($subject)), stripslashes($messageBody), "$from_name <$from_email>", "text/html", "", "", $error, '', '', $from_email);
				...
				foreach ($sitemgr_emails as $sitemgr_email) {
					system_mail($sitemgr_email, "[".EDIRECTORY_TITLE."] ".system_showText(LANG_NOTIFY_CONTACTUS)." - ".stripslashes(htmlspecialchars_decode($subject)), stripslashes($messageBody), "$from_name <$from_email>", "text/html", "", "", $error, '', '', $from_email);
					if ($error) $errors[] = system_showText(LANG_CONTACTMSGFAILED).($error ? '<br />'.$error : '');
				}
			}
			if (!count($errors)) {

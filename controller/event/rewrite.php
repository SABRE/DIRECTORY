<?
            ...
            elseif($countTempParts == 2)
            {
                ...
                if (mysql_num_rows($result) > 0) 
                {
                    ...
                    if (mysql_num_rows($result) > 0) {
                        ...
                        $sql = "SELECT id FROM EventCategory WHERE friendly_url = '" . $tempParts[1] . "' AND enabled = 'y' LIMIT 1";
                        $result = $dbObj->query($sql);
                        if (mysql_num_rows($result) > 0) {
                            $aux = mysql_fetch_assoc($result);
                            $_GET["category_id"] = $aux["id"];
    ...
    if ($_GET["category3"] && $_GET["category_id"] && !$failure) { 
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." friendly_url = ".db_formatString($_GET["category3"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;

	...
	if ($type == 'byId') {
		if ($childLevel) {
			...
			if ($retrieved_locations) {
				$return = "<option id=\"l_location_filter_".$childLevel."\" value=\"\"></option>";
				foreach ($retrieved_locations as $each_location) {
					$location_id = $each_location['id'];
					$location_name = $each_location['name'];
					$return .= "<option id=\"option_L_filter".$childLevel."_ID".$location_id."\" value=\"".$location_id."\">".trim($location_name)."</option>";
				}
			} else $return = "empty";
		}
		else $return = "empty";
		...
		if ($retrieved_locations) {
			$return = "<option id=\"l_location_filter_".$level."\" value=\"\"></option>";
			foreach ($retrieved_locations as $each_location) {
				$location_id = $each_location['id'];
				$location_name = $each_location['name'];
				$return .= "<option id=\"option_L_filter".$level."_ID".$location_id."\" value=\"".$location_id."\">".trim($location_name)."</option>";
			}
		} else $return = "empty";
	}	
	echo $return;

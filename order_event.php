<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($validate_account && $validate_contact && $validate_event && $validate_discount) {
			...
			if ($payment_method == "checkout") {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=event&item_level=".$level."&checkout=true&payment_method=checkout&item_id=".$event->getNumber("id");
				} else {
				...
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=event&item_level=".$level."&checkout=false&payment_method=invoice&item_id=".$event->getNumber("id");
				} else {
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/invoice.php";
				}
			} else {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=event&item_level=".$level."&checkout=false&payment_method=".$payment_method."&item_id=".$event->getNumber("id");
				} else {
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".MEMBERS_ALIAS."/login.php?destiny=".EDIRECTORY_FOLDER."/".MEMBERS_ALIAS."/".EVENT_FEATURE_FOLDER."/eventlevel.php";

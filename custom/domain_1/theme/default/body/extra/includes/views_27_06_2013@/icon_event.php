<?
	...
	$shareTemplate = "<a ".($event->getNumber("id") ? "id=\"link_social_".htmlspecialchars($event->getNumber("id")).$type."\"" : "")." href=\"javascript:void(0);\" onclick=\"enableSocialBookMarking(".$event->getNumber("id").", '".$type."', '".DEFAULT_URL."');\" $socialbookmarking_style title=\"Share this page\"></a>";
	...
	if ($members) {
		if (($id == sess_getAccountIdFromSession()) || ($members != "profile")) {
			...
			$favoriteTemplate = "<a id=\"favoritesRemove_".$event->getNumber("id")."\" href=\"".$remove_favorites_link."\" ".$remove_favorites_click." ".$remove_favorites_style." title=\"Add to favorites\"></a></li>";
		}
	}else {
		...
		$favoriteTemplate = "<a ".($event->getNumber("id") ? "id=\"favorites_".$event->getNumber("id")."\"" : "")." href=\"".$include_favorites_link."\" class=\"".$includes_favorites_class."\" ".$include_favorites_click." ".$include_favorites_style." title=\"Add to favorites\"></a></li>";
	}

<?
	...
	if (CLAIM_FEATURE == "on") {
		if (!htmlspecialchars($listing["account_id"])) { 
			if (htmlspecialchars($listing["claim_disable"]) == "n") { 
				...
				$listingtemplate_claim = "<a href=\"".$claim_link."\" ".$claim_style.">".$claim_image."</a>";
				//$listingtemplate_claim = "<a href=\"".$claim_link."\" ".$claim_style.">".$claim_textlink_string."</a>";
			}
		}
	}
    ...
    if (is_array($array_fields) && in_array("main_image", $array_fields)){
        ...
        if ($tPreview) {
            ...
            if ($is_object) {
                ...
                $imageObjT = new Image($listing["image_id"]);
                $auxPrefix = $imageObjT->prefix;
                unset($imageObjT);

                $arrImage["id"] = htmlspecialchars($listing["image_id"]);
            ...
            if ($imageObj->imageExists()) {
                if (($user) && ($levelObj->getDetail(htmlspecialchars($listing["level"])) == "y") || $levelObj->getActive(htmlspecialchars($listing["level"])) == 'n') {
                    ...
                    $listingtemplate_image .= $imageObj->getTag(true, IMAGE_LISTING_THUMB_WIDTH, IMAGE_LISTING_THUMB_HEIGHT, $listing["title"], true);
	...
	if(strlen($listing["title"])>20){
			$listing["title"]=substr($listing["title"],0,25).'...';	
	}
	...
	{ 
		...
		if ((!(($location_3!="" && $location_3==$listing["location_3"])||($location_4!="" && $location_4==$listing["location_4"]))) && zipproximity_getLocationDistanceLabel($lat, $long,  "listing", htmlspecialchars($listing["id"]), $distance_label, true, $listing)) {
			$listingtemplate_title .= " (".$distance_label.")";
		}
	}
	...
	if ($tPreview){
		...
		if ($is_object){ 
			...
			/*New Code */
			//$array_location_string=array($array_location_string[0],$array_location_string[1],$array_location_string[3]);
			/*End New Code*/
	...
	if(strlen($listing["description"])>200){
		$listing["description"]=substr($listing["description"],0,200).'...';
	}
	...
	if (htmlspecialchars($listing["email"]) && (is_array($array_fields) && in_array("email", $array_fields))) {
        ...
        if ($user){
			...
			$listingtemplate_email = "<a href=\"".DEFAULT_URL."/popup/popup.php?pop_type=listing_emailform&amp;id=".htmlspecialchars($listing["id"])."&amp;receiver=owner\" class=\"iframe fancy_window_tofriend\">Send an Email</a>";
		} else {
			//$listingtemplate_email = "<a href=\"javascript:void(0);\" style=\"cursor:default\">".system_showText(LANG_SEND_AN_EMAIL)."</a>";
			$listingtemplate_email = "<a href=\"javascript:void(0);\" style=\"cursor:default\">Send an Email</a>";
		}
	}
	...
	if ($review_enabled == "on" && $commenting_edir) {
        if ($levelsWithReview) {
            if (in_array($listing["level"], $levelsWithReview)) {
                ...
                $itemObj   = $listing;
                include($includeUrl."includes/views/view_review.php");
    ...
    */
	...
	$summaryFileName = $includeUrl."includes/views/view_listing_summary_code.php";
	
    $themeSummaryFileName = INCLUDES_DIR."/views/view_listing_summary_code_".EDIR_THEME.".php";

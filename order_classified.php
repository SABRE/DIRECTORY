<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($validate_account && $validate_contact && $validate_classified && $validate_discount) {
			...
			if ($payment_method == "checkout") {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=classified&item_level=".$level."&checkout=true&payment_method=checkout&item_id=".$classified->getNumber("id");
				} else {
				...
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=classified&item_level=".$level."&checkout=false&payment_method=invoice&item_id=".$classified->getNumber("id");
				} else {
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/invoice.php";
				}
			} else {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=classified&item_level=".$level."&checkout=false&payment_method=".$payment_method."&item_id=".$classified->getNumber("id");
				} else {
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".MEMBERS_ALIAS."/login.php?destiny=".EDIRECTORY_FOLDER."/".MEMBERS_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/classifiedlevel.php";

...
<?
    ...
    if ($advanced_search) 
    {
        ...
        if ($_default_locations_info) 
        {
            ...
            foreach ($_default_locations_info as $_default_location_info) 
            {
                if ($_default_location_info["show"] == "y") 
                {
                    ...
                    {
                        $show_legend = true;
                        ?><div class="search-form"><?= system_showText(LANG_SEARCH_LABELLOCATION) ?></div><?
                    }
        ...
        { 
            ...
            <div  class="dropdown-bg" id="div_location_<?= $_location_level ?>" <?= (${"locations" . $_location_level} && ($_location_father_level !== false ? ${"location_" . $_location_father_level} : true) || ($has_default_location && $_location_level == ($last_default_location + 1) && ${"locations" . $_location_level}) ) ? "" : "style=\"display:none;\"" ?>>
            <? if ($use_sitemgr_function) { ?>
                ...
                <? } else { ?>
                        <select  class="select" name="location_<?= $_location_level ?>" id="location_<?= $_location_level ?>" <? if ($_location_child_level) { ?> onchange="{ prev_filtered = filtered;filtered = false;loadLocation('<?= DEFAULT_URL ?>', '<?= EDIR_LOCATIONS ?>', <?= $_location_level ?>, <?= $_location_child_level ?>, this.value, <?= ($firstLoc ? "true" : "false") ?>);}" <? } else { ?> onchange="{ prev_filtered = filtered;filtered = false;fillLocations('<?= EDIR_LOCATIONS ?>');}"<? } ?>>-->
    ...
    if ($_default_locations_info) {
        ...
        foreach ($_default_locations_info as $_default_location_info) {
            ...
            ?>
            <input type="hidden" name="location_filter_<?= $_default_location_info["type"] ?>" value="<?= $_default_location_info["id"] ?>" /><?
            $last_default_location = $_default_location_info["type"];
        }
        $locations_default_where = string_substr($locations_default_where, 0, -2);
        $locations_default_where_replace = "yes";
        ...
        ?>
        <input type="hidden" name="locations_filter_default_where" id="locations_filter_default_where" value="<?= $locations_default_where ?>" />	
        <input type="hidden" name="locations_filter_default_where_replace" id="locations_filter_default_where_replace" value="<?= $locations_default_where_replace ?>" />
        <?
    }
    ...
    if ($_non_default_locations) {
        ...
        foreach ($_non_default_locations as $_location_level) {
            ...
            <div class="field loading-location" id="div_img_loading_filter_<?= $_location_level ?>" style="display:none;">
                <img src="<?= DEFAULT_URL ?>/theme/<?= EDIR_THEME ?>/images/iconography/icon-loading-location.gif" alt="<?= system_showText(LANG_WAITLOADING) ?>"/>
            </div>
            <div class = "dropdown-bg" id="div_location_filter_<?= $_location_level ?>" <?= (${"locations" . $_location_level} && ($_location_father_level !== false ? ${"location_" . $_location_father_level} : true) || ($has_default_location && $_location_level == ($last_default_location + 1) && ${"locations" . $_location_level}) ) ? "" : "style=\"display:none;\"" ?>>
            <? if ($use_sitemgr_function) { ?>
                    <select name="location_filter_<?= $_location_level ?>" id="location_filter_<?= $_location_level ?>" <? if ($_location_child_level) { ?> onchange="loadLocationSitemgrMembersFilter('<?= DEFAULT_URL ?>', '<?= EDIR_LOCATIONS ?>', <?= $_location_level ?>, <?= $_location_child_level ?>, this.value);fillLocations('<?= EDIR_LOCATIONS ?>');" <? } else { ?> onchange="fillLocations('<?= EDIR_LOCATIONS ?>');"<? } ?>>
            <? } else { ?>
                        <select class="select" name="location_filter_<?= $_location_level ?>" id="location_filter_<?= $_location_level ?>" <? if ($_location_child_level) { ?> onchange="loadLocationFilter('<?= DEFAULT_URL ?>', '<?= EDIR_LOCATIONS ?>', <?= $_location_level ?>, <?= $_location_child_level ?>, this.value, <?= ($firstLoc ? "true" : "false") ?>);" <? } else { ?> onchange="fillLocations('<?= EDIR_LOCATIONS ?>')"<? } ?>>
                <? } ?>
                        <option id="l_location_filter_<?= $_location_level ?>" value=""><?= system_showText(constant("LANG_SEARCH_LABELCB" . constant("LOCATION" . $_location_level . "_SYSTEM"))) ?></option>
                <?
                if (${"locations" . $_location_level}) { 
                    foreach (${"locations" . $_location_level} as $each_location) {
                        if(${"location_" . $_location_level} != 1){
                            $selected = (${"location_" . $_location_level} == $each_location["id"]) ? "selected" : "";
                        }else{
                            $selected='';
                        }
            ...
            $firstLoc = false;
                ?>
                    ...
                    <div class="field" id="box_no_location_filter_found_<?= $_location_level ?>" style="display: none;">&nbsp;</div>
            </div>
                    ...
                    ?>
        <span class="clear-location" id="locations_filter_clear" <?= ($showClear ? "" : "style=\"display: none\"" ) ?>>
            <a href="javascript:void(0);" class="view-button" onclick="clearLocationsFilter('<?= EDIR_LOCATIONS ?>', <?= $has_default_location ? $has_default_location : 0 ?>, <?= $last_default_location ? $last_default_location : 0 ?>);">
        <?= system_showText(LANG_BUTTON_CLEAR) ?>
            </a>
        </span>
        <?
    ...
    if ($_default_locations_info) {
        foreach ($_default_locations_info as $_default_location_info) {
            if ($_default_location_info["show"] == "y") {
                if (($_default_location_info["type"] < 4 && string_strpos($_SERVER["REQUEST_URI"], BANNER_FEATURE_FOLDER)) || (string_strpos($_SERVER["REQUEST_URI"], BANNER_FEATURE_FOLDER) == false)) {
                    ?>
                            <tr>
                                ...
                                <td colspan="2"> <?= (($_location_level < 4 && string_strpos($_SERVER["REQUEST_URI"], BANNER_FEATURE_FOLDER)) ? "* " : "") . $_default_location_info["name"] ?> </td>
                                <td>&nbsp;</td>
                            </tr>
                            <?
                        }
            ...
            if ($_non_default_locations) {
                foreach ($_non_default_locations as $_location_level) {
                    ...
                    if (($_location_level < 4 && string_strpos($_SERVER["REQUEST_URI"], BANNER_FEATURE_FOLDER)) || (string_strpos($_SERVER["REQUEST_URI"], BANNER_FEATURE_FOLDER) == false)) {
                        ?>
                        ...
                        <tr id="div_location_<?= $_location_level ?>" <?= (${"locations" . $_location_level} && $_POST["new_location" . $_location_level . "_field"] == "" /* && ($_location_father_level !== false? ${"location_".$_location_father_level} : true) */) ? "" : "style=\"display:none;\"" ?>>
                            <th>
                                <label for="location_<?= $_location_level ?>"><?= (($_location_level < 4 && string_strpos($_SERVER["REQUEST_URI"], BANNER_FEATURE_FOLDER)) ? "* " : "") . $location_name ?>:</label>
                            </th>

	...
	if ( $advanced_search ) { ?> 	
		...
		if ($_non_default_locations) {
			...
			foreach($_non_default_locations as $_location_level) {
				...
				<div id="div_location_filter_<?=$_location_level?>" <?=(${"locations".$_location_level} && ($_location_father_level !== false? ${"location_".$_location_father_level} : true) || ($has_default_location && $_location_level == ($last_default_location + 1) && ${"locations".$_location_level}) ) ? "" : "style=\"display:none;\""?>>
					<?
					if($use_sitemgr_function){ ?>

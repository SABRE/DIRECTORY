 	...
 	<table border="1" cellpadding="2" cellspacing="2" align="center" class="standard-tableTOPBLUE">
		...
		<? if ($commentsArr) foreach($commentsArr as $each_rate) {
			...
			<tr>
				...
				<td>
					<a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BLOG_FEATURE_FOLDER;?>/comments/view.php?post_id=<?=$post->getNumber("id")?>&id=<?=$each_rate->getString("id")?>&screen=<?=$screen?>&letter=<?=$letter?>&item_screen=<?=$item_screen?>&item_letter=<?=$item_letter?>" class="link-table">
					...
					<? if (!$reply_title) { ?>
						...
						<? } else { ?>
							<a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BLOG_FEATURE_FOLDER;?>/comments/index.php?reply_id=<?=$each_rate->getNumber("id")?>" class="link-table">
					...
					<? } ?>
					<? } ?>
					<a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BLOG_FEATURE_FOLDER;?>/comments/delete.php?post_id=<?=$post->getNumber("id")?>&id=<?=$each_rate->getString("id")?>&screen=<?=$screen?>&letter=<?=$letter?>&item_screen=<?=$item_screen?>&item_letter=<?=$item_letter?>" class="link-table">

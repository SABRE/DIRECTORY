...
?>
    ...
    if (is_numeric($message) && isset($msg_event[$message])) {
        ...
        echo "<p class=\"successMessage\">".$msg_maptunning[$message_maptunning]."</p>";
    }
    ...
    <form name="item_table">
        ...
        <table border="0" cellpadding="0" cellspacing="0" class="standard-tableTOPBLUE arrow-manage">
            ...
            <tr>
                <th style="width:auto;">
                    <span style="width:auto;"><?=system_showText(LANG_EVENT_TITLE)?></span>
                    <? if ($orderLinks) { ?>
                ...
                <th style="width: 60px;">
                    <span style="width: 48px;"><?=system_showText(LANG_LABEL_LEVEL)?></span>
                    <? if ($orderLinks) { ?>
                        <a href="<?=$paging_url."?order_by=level_".($order_by == "level_asc" ? "desc" : "asc")."&letter=$letter&screen=$screen".($url_search_params ? "&$url_search_params" : "")?>">
                            <img src="<?=DEFAULT_URL."/images/bg_arrow_".($order_by == "level_asc" ? "up" : "down").".png"?>" title="<?=system_showText(@constant("LANG_CLICK_ORDERLEVEL".($order_by == "level_asc" ? "DESC" : "ASC")))?>" />
                        </a>
                    <? } ?>
                </th>

                <th style="width: 105px;">
                    <span style="width: 93px;"><?=system_showText(LANG_LABEL_START_DATE)?></span>
                    <? if ($orderLinks) { ?>
                        <a href="<?=$paging_url."?order_by=startdate_".($order_by == "startdate_asc" ? "desc" : "asc")."&letter=$letter&screen=$screen".($url_search_params ? "&$url_search_params" : "")?>">
                            <img src="<?=DEFAULT_URL."/images/bg_arrow_".($order_by == "startdate_asc" ? "up" : "down").".png"?>" title="<?=system_showText(@constant("LANG_CLICK_ORDERSTARTDATE".($order_by == "startdate_asc" ? "DESC" : "ASC")))?>" />
                        </a>
                    <? } ?>
                </th>
                ...
                <? if (string_strpos($url_base, "/".SITEMGR_ALIAS."")) { ?>
                    <th style="width: 105px;">
                        <span style="width: 93px;"><?=system_showText(LANG_LABEL_ACCOUNT);?></span>
                    ...
                    <th style="width: 105px;">
                        ...
                        <? if ($orderLinks) { ?>
                            <a href="<?=$paging_url."?order_by=renewal_".($order_by == "renewal_asc" ? "desc" : "asc")."&letter=$letter&screen=$screen".($url_search_params ? "&$url_search_params" : "")?>">
                                <img src="<?=DEFAULT_URL."/images/bg_arrow_".($order_by == "renewal_asc" ? "up" : "down").".png"?>" title="<?=system_showText(@constant("LANG_CLICK_ORDERRENEWAL".($order_by == "renewal_asc" ? "DESC" : "ASC")))?>" />
                            </a>
                        <? } ?>
                    </th>
                <? } ?>         
                ...
                <th style="width: <?=(string_strpos($url_base, "/".SITEMGR_ALIAS."") ? "145px;" : "115px;")?>">
            ...
            if ($events) foreach ($events as $event) {
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <? if ((GOOGLE_MAPS_ENABLED == "on") && ($mapObj->getString("value") == "on") && (($event->getString("address")) || ($event->getNumber("location_3")) || ($event->getNumber("location_4")) || ($event->getNumber("zip_code"))  || ($event->getString("latitude") && $event->getString("longitude")))) { ?>
                        ...
                        <? if ( PAYMENTSYSTEM_FEATURE == "on" ) { ?>
                            ...
                            <? } elseif(string_strpos($url_base, "/".SITEMGR_ALIAS."")) { ?>

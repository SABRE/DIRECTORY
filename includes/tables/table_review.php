	...
	<table border="1" cellpadding="2" cellspacing="2" align="center" class="standard-tableTOPBLUE">
		...
		<? if ($reviewsArr) foreach($reviewsArr as $each_rate) { ?>
			...
			<tr>
                ...
                <? if (string_strpos($url_base, "/".SITEMGR_ALIAS."") && ($each_rate->getNumber("approved") == 0 || (string_strlen(trim($each_rate->getString("response"))) > 0 && $each_rate->getNumber("responseapproved") == 0))) { ?> <a href='javascript:void(0);' onclick='showStatusField(<?=$each_rate->getNumber('id');?>);'> <? } ?>
                    <?
                    ...
                    ?>
                <? if (string_strpos($url_base, "/".SITEMGR_ALIAS."") && ($each_rate->getNumber("approved") == 0 || (string_strlen(trim($each_rate->getString("response"))) > 0 && $each_rate->getNumber("responseapproved") == 0))) { ?> </a> <? } ?>      
				...
				<td>
					...
					<? if (string_strpos($url_base, "/".SITEMGR_ALIAS."")) { ?>
						...
						<a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/review/delete.php?item_id=<?=$each_rate->getString("item_id")?>&item_type=<?=$each_rate->getString("item_type")?><?=($filter_id ? "&filter_id=1" : '')?>&id=<?=$each_rate->getString("id")?>&screen=<?=$screen?>&letter=<?=$letter?>&item_screen=<?=$item_screen?>&item_letter=<?=$item_letter?>" class="link-table">

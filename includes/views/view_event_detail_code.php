...
?>
	...
	<div class="columns">
		...
		<div class="right">
		<p class="event-when"><strong><?=system_showText(LANG_EVENT_WHEN);?>:</strong> <?=($event->getString("recurring") != "Y" ? $str_date : $str_recurring);?></p>

				<? if ($str_time) { ?>
                    <p class="event-time"><strong><?=system_showText(LANG_EVENT_TIME)?>:</strong> <?=$str_time?></p>
				<? } ?>
		</div>
	</div>

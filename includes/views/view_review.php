<?
    ...
    if ($review_enabled == "on" && $commenting_edir) {
		...
		//$aux_item_review .= "</div>";
            ...
            if (!LISTING_REVIEW_SUMMARY){ 
                $reviewImageLink = "<img src=\"".DEFAULT_URL."/custom/domain_2/theme/default/schemes/default/images/newImages/write_a_review.png\" />";
                if ($user) { 
                    if (!$bt_rate_off){
                        //$aux_item_review .= "<p>";
                        $aux_item_review .= "<span id='review-link'>";
                        $reviewLinkDetail = "<span id='review-link'>";
                        if ($review_amount > 0) {
                            /** Rate it! */
                            ...
                            if($_SESSION['check_member_logged'] == 'checked'){
                                $reviewLinkDetail .= $reviewImageLink;
                            }else{
                                $reviewLinkDetail .= "<a href=\"".$linkReviewFormPopup."\" class=\"$class rate-it\" >" .$reviewImageLink. "</a>";
                            }
                        } else {
                            ...
                            if($_SESSION['check_member_logged'] == 'checked'){
                                ...
                                $reviewLinkDetail .= "<a href=\"".$linkReviewFormPopup."\" class=\"$class\">" .$reviewImageLink."</a>";
                            }
                        }
                    }
                    //$aux_item_review .= "</p>";
                    $aux_item_review .='</span>';
                    $reviewLinkDetail .= "</span>";
                }
            } else {
                if (!$bt_rate_off){
                    // $aux_item_review .= "<p>";
                    ...
                    $reviewLinkDetail = "<span id='review-link'>";
                     if ($review_amount > 0) {
                         ...
                         $reviewLinkDetail .= "<a href='javascript:void(0);'  style='cursor: default'>" . $reviewImageLink . "</a>";

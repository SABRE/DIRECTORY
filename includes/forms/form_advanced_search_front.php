<?
	...
	if (${"locations".$_non_default_locations[0]} || $showLoc) { ?>
		<div class="left">
			...
			<div>
                    <input type="text" onkeyup="checkValidDist(this)" name="dist_loc" id="dist_top_loc" value="" class="text" />
	...
	if ($hasWhereSearch) { ?>
		...
		<div class="left zipcode">
			...
			<? if (ZIPCODE_PROXIMITY == "on") { ?>
                <div>
                    <input type="text" name="dist" id="dist_top" value="" class="text" />
                    <?=string_ucwords(ZIPCODE_UNIT_LABEL_PLURAL)." ".system_showText(LANG_SEARCH_LABELZIPCODE_OF)?>
                </div>
            <? } ?>
			<div>
				<input type="text" name="zip" id="zip_top" value="" class="text" />

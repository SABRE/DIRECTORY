...
<script type="text/javascript">
	...
	function showLocation(obj) {
		if (obj.checked == true) {
			document.getElementById("featureLocation").style.display = '';
		} else {
			document.getElementById("featureLocation").style.display = 'none';
			document.getElementById("location_1").value='';
			document.getElementById("location_1").onchange();
		}
	}
...
<? //////////////////////////////////////////////////////////////////////////////////// ?>
	...
	<table cellpadding="0" cellspacing="0" border="0" class="standard-table nomargin">
		...
		<tr>
			...
			<td nowrap="nowrap" class="banner-section-type">
					<span><input <?=($type==4?"disabled=\"true\"":"")?> type="radio" id="section" name="section" value="general" checked="checked"  onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');" class="inputAlign" /> <?=system_showText(LANG_GENERALPAGES);?></span>
					<span><input type="radio" id="section" name="section" value="listing" <? if ($section == "listing" || ($type==4 && $section=="general")) echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');" class="inputAlign" /> <?=system_showText(LANG_LISTING_FEATURE_NAME);?></span>
                ...
                <? if ( string_strpos($url_base, "/".SITEMGR_ALIAS."") ) { ?>
                    <span><input <?=($type==4?"disabled=\"true\"":"")?> type="radio" id="section" name="section" value="global" <? if ($section == "global") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');" class="inputAlign" /> <?=system_showText(LANG_SITEMGR_BANNER_GLOBAL);?></span>
                <? } ?>
			</td>
		</tr>
		<tr>
		...
		<tr id="approveFeature" <?=(($type==4)?"":"style=\"display:none\"")?>>
			<th class="wrap" style="vertical-align:top"><?=system_showText(LANG_FEATURE_BANNERINLOCATION)?>:</th>
			<td>
				<?=($approve_feature=="D"?"<span style=\"text-align: justify;\">".system_showText(LANG_LABEL_DENIED_APPLICATION)."</span>":"")?>
				<?=($approve_feature=="O"?"<span style=\"text-align: justify;\">".system_showText(LANG_LABEL_APPROVED_APPLICATION)."</span>":"")?>
                <input type="checkbox" name="approve_feature" value="P" <?=($approve_feature=="P" || $approve_feature=="D" ||$approve_feature=="O") ? "checked" : "";?> class="inputAlign" onchange="showLocation(this)"/>
            	<span style="text-align: justify;"><?=system_showText(LANG_FEATURE_BANNERINLOCATIONNOTE);?></span>
            	<span style="text-align: justify;"><?=system_showText(LANG_FEATURE_BANNERINLOCATIONNOTE2);?></span>
            </td>
		</tr>
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="standard-table" id="banner_with_images">
        ...
        <tr>
            <th style="vertical-align:top">* <?=system_showText(LANG_LABEL_FILE)?>:</th>
            <td>
                <input type="file" name="file" class="input-form-banner" size="60" />
        ...
        <? if ($image_id > 0 && $id) { ?>
            <tr>
                <th>&nbsp;</th>
                <td class="standard-tableContent" style="text-align: center; display:table-cell;">

...
<table id="account_info" border="0" cellpadding="2" cellspacing="0" class="standard-table noMargin">
	...
	<? if ((string_strpos($username, "facebook::") === false && string_strpos($username, "openid::") === false && string_strpos($username, "google::") === false) || $sitemgrsection) { ?>
		...
		<tr>
			<th<?=(($sitemgrsection && ($id || $account_id) && $isForeignAcc) ? "" : $th_class);?>>* <?=system_showText(LANG_LABEL_USERNAME)?>:</th>
			<td>
				<? if ($sitemgrsection && ($id || $account_id)&& $isForeignAcc) { ?>
	...
	<? if (!$isForeignAcc) { ?>
		...
		<? if (string_strpos($_SERVER["PHP_SELF"], "/".MEMBERS_ALIAS."/account") !== false || string_strpos($_SERVER["PHP_SELF"], "/".SOCIALNETWORK_FEATURE_NAME."/edit") !== false) { ?>
			<tr>

...
<? if ((!$members) && (CLAIM_FEATURE == "on")) { ?>
	<br />
	<div class="tip-base">
		<p style="text-align: justify;"><a href="<?=DEFAULT_URL;?>/<?=SITEMGR_ALIAS?>/faq/faq.php?keyword=<?=urlencode("claim");?>" target="_blank"><?=system_showText(LANG_SITEMGR_LISTING_TIPCLAIM_TITLE)?></a><br /><br /><br /><span style="float: right;"><?=system_showText(LANG_SITEMGR_LISTING_TIPCLAIM_1)?> <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/prefs/claim.php"><?=system_showText(LANG_SITEMGR_LISTING_TIPCLAIM_2)?></a>.</span></p>
	</div>
	<br />
<? } ?>
        ...
        <table cellpadding="0" cellspacing="0" border="0" class="standard-table">
            ...
            <? } ?>
                <td class="alignTop width100">
                    ...
                    <? } else { ?>
                        <a id="uploadimage" href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/uploadimage.php?gallery_hash=<?=$gallery_hash?>&item_type=listing&item_id=<?=$listing->getNumber("id")?>&galleryid=<?=$gallery_id[0]?>&photos=<?=$levelMaxImages?>&level=<?=$level?>" class="addImageForm input-button-form iframe fancy_window_imgAdd"><b><?=system_showText(LANG_LABEL_ADDIMAGE)?></b></a>
                    <? } ?>
...
<? } ?>
    ...
    <? if(is_array($array_fields) && in_array("attachment_file", $array_fields)){ ?>
        <table cellpadding="0" cellspacing="0" border="0" class="standard-table">
            <tr>
                <th colspan="2" class="standard-tabletitle"><?=system_showText(LANG_LABEL_ATTACH_ADDITIONAL_FILE)?> <span style="display:block;white-space:normal;">(PDF, DOC, TXT, JPG, GIF <?=system_showText(LANG_OR);?> PNG)</span></th>
            </tr>
            <?
	...
	<table cellpadding="0" cellspacing="0" border="0" class="standard-table">
        ...
        <? if (is_array($array_fields) && in_array("summary_description", $array_fields)) { ?>
            ...
            <tr>
                <td>
                    <textarea id="summarydesc" name="description" rows="5" cols="1"><?=$description;?></textarea>
                </td>
            </tr>
        <? } ?>
	...
	<? if (is_array($array_fields) && in_array("long_description", $array_fields)) { ?>
        <table cellpadding="0" cellspacing="0" border="0" class="standard-table">
            <tr>
                ...
                <tr>
                    <td>
                        <textarea name="long_description" rows="5"><?=$long_description?></textarea>
                    </td>
                </tr>
        </table>
    <? } ?>
							...
							<tr>
								...
								<td>
									...
									if ($imageObj->imageExists()) {
										echo $imageObj->getTag(IS_UPGRADE == "on" ? true : false, IMAGE_DESIGNATION_WIDTH, IMAGE_DESIGNATION_HEIGHT, $editor->getString("name", false));
									}
									?>
								</td>
								<?
...
<? if (((!$listing->getNumber("id")) || $listing->getNumber("package_id")>0 || (($listing) && ($listing->needToCheckOut())) || (string_strpos($url_base, "/".SITEMGR_ALIAS."")) || (($listing) && ($listing->getPrice() <= 0))) && ($process != "signup")) { ?>
	...
	<script language="javascript" type="text/javascript">
		<? if ($listingtemplate_id) { ?>
			loadCategoryTree('template', 'listing_', 'ListingCategory', 0, <?=$listingtemplate_id?>, '<?=DEFAULT_URL."/".EDIR_CORE_FOLDER_NAME."/".LISTING_FEATURE_FOLDER?>',<?=SELECTED_DOMAIN_ID?>);
		<? } else { ?>

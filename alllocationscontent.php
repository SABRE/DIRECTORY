...
?>
	...
	if ($locationsToShow) {
        ...
        $location_url_base = "/".ALIAS_LOCATION_URL_DIVISOR."/";
		...
		if (EDIR_DEFAULT_LOCATIONS) {
			...
			foreach($defaultLocations as $_location_level) {
				...
				$location_url_base = "/".ALIAS_LOCATION_URL_DIVISOR."/".($location_path["friendly"] === false ? '' : implode("/",$location_path["friendly"])."/").${"location".$_location_level}[0]["friendly_url"];

...
?>
    ...
    <div class="user-info">
        ...
        <? } else if ($id == sess_getAccountIdFromSession() && $info["is_sponsor"] == 'y') { ?>
                <p class="edit"><a href="<?=DEFAULT_URL;?>/<?=MEMBERS_ALIAS?>/account/account.php?id=<?=sess_getAccountIdFromSession()?>" title="<?=LANG_LABEL_EDITPROFILE;?>"><?=LANG_LABEL_EDITPROFILE;?></a></p>
        <? } ?>
        ...
        <? if (!$info["facebook_image"]) {
            ...
            <? } ?>
            <p>
                <?=htmlspecialchars($info["nickname"]);?>

                <? if ($info["country"] && $publish == "y") { ?>
                    <span>
                        <?=htmlspecialchars($info["country"]); ?>
                    </span>
                <? } ?>

                <? if ($info["birth_date"] != '0000-00-00') { ?>
                <span>
                    <?=system_showText(LANG_LABEL_BIRTHDATE).": ".format_date($info["birth_date"])?>
                </span>
                <? } ?>
            </p>

<?
	...
	if(isset($_GET["location_3"]) && $_GET["location_3"]!="")
			{	
					unset($locObj);
					$locObj = new Location3($_GET["location_3"]);
					$locObj->setCoordinates();
			}
			if(isset($_GET["location_4"]) && $_GET["location_4"]!="")
			{
					unset($locObj);
					$locObj = new Location4($_GET["location_4"]);
					$locObj->setCoordinates();
			}
					...
					if ($categories) {
						...
						foreach ($categories as $category) {
							...
							if ($thislistings > 0) {
								?>
								<li class="level-2">
									<span class="level-1 level-title">&nbsp;</span><a href="<?=LISTING_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>&amp;category_id=<?=$this_category_id?>"><?=$category->getString("title")?></a> <span>(<?=$thislistings?>)</span>
								</li>
								<?
							}

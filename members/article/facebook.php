<?
	...
	if (!$commenting_fb){
		header("Location: ".DEFAULT_URL."/".MEMBERS_ALIAS."/".ARTICLE_FEATURE_FOLDER."/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
		exit;
	}
	...
	if ($id) {
		...
		if (sess_getAccountIdFromSession() != $article->getNumber("account_id")) {
			header("Location: ".DEFAULT_URL."/".MEMBERS_ALIAS."/".ARTICLE_FEATURE_FOLDER."/index.php?screen=$screen&letter=$letter");
			exit;
		}
	} else {
		header("Location: ".DEFAULT_URL."/".MEMBERS_ALIAS."/".ARTICLE_FEATURE_FOLDER."/index.php?screen=$screen&letter=$letter");
...
?>
    ...
    <div class="content">
        ...
        <form name="facebook_form" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
            ...
            <?
            $detailLink = ARTICLE_DEFAULT_URL."/".ALIAS_SHARE_URL_DIVISOR."/".$article->getString("friendly_url").".html";

<?
	...
	if ($id) {
		...
		if ((!$event->getString("address")) && (!$event->getString("zip_code")) && (!$event->getNumber("location_3")) && (!$event->getNumber("location_4")) && (!$event->getString("latitude")) && (!$event->getNumber("longitude"))) {
			header("Location: ".DEFAULT_URL."/".MEMBERS_ALIAS."/".EVENT_FEATURE_FOLDER."/index.php?screen=$screen&letter=$letter");
			exit;
		}
		$itemObj = $event;
...
?>
	...
	<div class="content">
		...
		<ul class="list-view">
			<li class="list-back"><a href="<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/<?=EVENT_FEATURE_FOLDER;?>/<?=($search_page) ? "search.php" : "index.php"?>?screen=<?=$screen?>&letter=<?=$letter?>"><?=system_showText(LANG_LABEL_BACK);?></a></li>
		</ul>
		...
		<form id="formeventmaptuningcancel" action="<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/<?=EVENT_FEATURE_FOLDER;?>/index.php" method="post">

<?
	...
	if ($id) {
		...
		if ((!$listing->getString("address")) && (!$listing->getString("zip_code")) && (!$listing->getNumber("location_3")) && (!$listing->getNumber("location_4")) && (!$listing->getString("latitude")) && (!$listing->getNumber("longitude"))) {
			header("Location: ".DEFAULT_URL."/".MEMBERS_ALIAS."/".LISTING_FEATURE_FOLDER."/index.php?screen=$screen&letter=$letter");
			exit;
		}
		$itemObj = $listing;
...
?>
	...
	<div class="content">
		...
		<form id="formlistingmaptuningcancel" action="<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/<?=LISTING_FEATURE_FOLDER;?>/index.php" method="post">

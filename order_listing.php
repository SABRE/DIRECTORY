<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST") && !$_POST["account_sugar_id"]) {
		...
		if ($validate_account && $validate_contact && $validate_listing && $validate_discount) {
			...
			if ($payment_method == "checkout") {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=listing&item_level=".$level."&checkout=true&payment_method=checkout&item_id=".$listing->getNumber("id");
				} else {
				...
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=listing&item_level=".$level."&checkout=false&payment_method=invoice&item_id=".$listing->getNumber("id");
				} else {
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/invoice.php";
				}
			} else {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=listing&item_level=".$level."&checkout=false&payment_method=".$payment_method."&item_id=".$listing->getNumber("id");
				} else {
	...
	$formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".MEMBERS_ALIAS."/login.php?destiny=".EDIRECTORY_FOLDER."/".MEMBERS_ALIAS."/".LISTING_FEATURE_FOLDER."/listinglevel.php";
								...
								<table width="100%" border="0" cellpadding="2" class="tableCategoriesADDED" cellspacing="0">
									...
									<tr>
										<td class="tableCategoriesBUTTONS" colspan="2">
											<button type="button" onclick="JS_displayCategoryPath(document.order_listing.feed, '<?=system_showText(LANG_MSG_SELECT_CATEGORY_FIRST)?>', '<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/<?=LISTING_FEATURE_FOLDER;?>', 'info_window', false, 300, 100);"><?=system_showText(LANG_CATEGORY_VIEWPATH)?></button>&nbsp;

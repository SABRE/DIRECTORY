...
<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /cron/update_location3_coordinates.php
	# ----------------------------------------------------------------------------------------------------
	...
	$sqlDomain = "	SELECT
					...
					WHERE CC.`running` = 'N'
					AND CC.`type` = 'update_location3_coordinates'
					AND D.`status` = 'A' "
//					AND (ADDDATE(CC.`last_run_date`, INTERVAL 1 DAY) <= NOW() OR CC.`last_run_date` = '0000-00-00 00:00:00')
	...
	var_dump(mysql_num_rows($resDomain));

	if (mysql_num_rows($resDomain) > 0) {
		$rowDomain = mysql_fetch_assoc($resDomain);
		define("SELECTED_DOMAIN_ID", $rowDomain["id"]);

		$sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'Y', `last_run_date` = NOW() WHERE `domain_id` = ".SELECTED_DOMAIN_ID." AND `type` = 'update_location3_coordinates'";
		mysql_query($sqlUpdate, $link);
        $messageLog = "Starting cron";
        log_addCronRecord($link, "update_location3_coordinates", $messageLog, false, $cron_log_id);
	...
	$sql = "SELECT id, name FROM Location_3 WHERE latitude = 0 OR longitude = 0 LIMIT 1000";
	...
	$sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'N' WHERE `domain_id` = ".SELECTED_DOMAIN_ID." AND `type` = 'update_location3_coordinates'";
	mysql_query($sqlUpdate, $link);

	////////////////////////////////////////////////////////////////////////////////////////////////////
	$time_end = getmicrotime();
	$time = $time_end - $time_start;
	print "Location3 Coordinates Update on Domain ".SELECTED_DOMAIN_ID." - ".date("Y-m-d H:i:s")." - ".round($time, 2)." seconds.\n";
	if (!setting_set("last_datetime_update_location3_coordinates", date("Y-m-d H:i:s"))) {
		if (!setting_new("last_datetime_update_location3_coordinates", date("Y-m-d H:i:s"))) {
			print "last_datetime_update_location3_coordinates error - Domain - ".SELECTED_DOMAIN_ID." - ".date("Y-m-d H:i:s")."\n";
            $messageLog = "Database error - LINE: ".__LINE__;
            log_addCronRecord($link, "update_location3_coordinates", $messageLog, true, $cron_log_id);
		}
	}
    $messageLog = "Cron finished";
    log_addCronRecord($link, "update_location3_coordinates", $messageLog, true, $cron_log_id, true, round($time, 2));
	////////////////////////////////////////////////////////////////////////////////////////////////////
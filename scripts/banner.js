...
var disableArr = new Array('general','article','blog','global');
...
function bannerCheckType(type){
	...
	if (type < 50) {
        ...
        if(type==4)
        {
        	$('#approveFeature').css('display', '');
        	for (var i = 0; i < disableArr.length; i++) {
            	var a = disableArr[i];
            	$("input[id='section'][value=" + a + "]").attr("disabled",true);
            }
        	$("input[id='section'][value=listing]").attr("checked",true);
        	$("input[id='section'][value=listing]").click();
        	$("input[name='approve_feature']").attr("checked",true);
        	$("input[name='approve_feature']").change();
        }else{
        	$('#approveFeature').css('display', 'none');
        	for (var i = 0; i < disableArr.length; i++) {
            	var a = disableArr[i];
            	$("input[id='section'][value=" + a + "]").attr("disabled",false);
            }
        	$("input[id='section'][value=general]").attr("checked",true);
        	$("input[id='section'][value=general]").click();
        	$("input[name='approve_feature']").attr("checked",false);
        	$("input[name='approve_feature']").change();
        }
	} else if (type >= 50) {

...
function containerReload(){
	var Content;
	if(typeof in_banner!= undefined && in_banner == true)
		var item = true;
	...
	if ($.browser.msie && $.browser.version == 6){
		...
		try{
			...
			if(filtered==true)
			{
				Content = document.getElementById('LocationbaseAdvancedSearchFilter').innerHTML;
				document.getElementById('LocationbaseAdvancedSearchFilter').innerHTML = "";
				document.getElementById('LocationbaseAdvancedSearchFilter').innerHTML = Content;
				filtered = false;
			}
			else
			{
...
function loadLocationSitemgrMembersFilter(url, edir_locations, level, childLevel, id) {
	...
	if (!isNaN(id)) {		
		...
		for (i=0; i<edir_locations.length; i++) {
			if (edir_locations[i]>level) {
				text = $("#l_location_filter_"+edir_locations[i]).attr("text");
				$("#location_filter_"+edir_locations[i]).html("<option id=\"l_location_filter_"+edir_locations[i]+"\" value=\"\">"+text+"</option>");
				$('#div_location_filter_'+edir_locations[i]).css('display', 'none');
				$('#new_location_filter_'+edir_locations[i]+'_field').attr('value', '');
				$('#div_new_location_filter_'+edir_locations[i]+'_field').css('display', 'none');				
			}
		}	

		$("#div_location_filter_"+childLevel).css("display","");
		$('#locations_filter_clear').css('display', '');
		$('#location_filter_'+childLevel).css('display', 'none');
		$('#div_img_loading_filter_'+childLevel).css('display', '');
		$('#box_no_location_filter_found_'+childLevel).css('display', 'none');
                try{
                    $('#div_select_filter_'+childLevel).css('display', 'none');
                } catch(e){}

		$.get(url+"/locationFilter.php",{id: id, level: level, childLevel: childLevel, type:'byId'},function(location){
			
			if (location!="empty"){
				var text = $("#l_location_filter_"+childLevel).attr("text");			
				$("#location_filter_"+childLevel).html(location);
				$("#l_location_filter_"+childLevel).html(text);
				$('#location_filter_'+childLevel).css('display', '');
                try{
                    $('#div_select_filter_'+childLevel).css('display', '');
                } catch(e){}
				display_level_limit = childLevel;
			} else {
				if (!id) 
					$("#div_location_filter_"+childLevel).css("display", 'none');
				else {
                    try{
                        $('#div_select_filter_'+childLevel).css('display', '');
                    } catch(e){}
			...
			if (childLevel && id)
				$('#div_new_location_filter_'+childLevel+'_link').css('display', '');
			else
				$('#div_new_location_filter_'+childLevel+'_link').css('display', 'none');			

			$('#div_img_loading_filter_'+childLevel).css('display', 'none');	
...
function loadLocationFilter(url, edir_locations, level, childLevel, id, showClear) {
	...
	if (!isNaN(id)) {		
		...
		$("#div_location_filter_"+childLevel).css("display","");
		$('#location_filter_'+childLevel).css('display', 'none');
		$('#location_filter_'+level).attr('disabled', 'true');
		$('#div_img_loading_filter_'+childLevel).css('display', '');
		if ($('#locations_filter_clear')){
		...
		$.get(url+"/locationFilter.php",{id: id, level: level, childLevel: childLevel, type:'byId'},function(location){
			...
			$('#location_filter_'+level).attr('disabled', '');	
			$('#div_img_loading_filter_'+childLevel).css('display', 'none');	
			if ($('#locations_filter_clear')){
				$('#locations_filter_clear').css('display', '');
			}
...
function fillLocations(levels){
	...
	if(edir_locations){
		...
		for (i=0; i<edir_locations.length; i++) {
			if(filtered==true)
			{
				if($("#location_filter_"+edir_locations[i]+" option:selected").val() > 0){
					fillFieldWhere($("#location_filter_"+edir_locations[i]+" option:selected").text());
				}
			}
			else
			{
...
function clearLocationsFilter(levels, has_default, last_default){
	...
	for (i=0; i<edir_locations.length; i++) {
		if (i > first_to_show) {
			...
			$('#div_location_filter_'+edir_locations[i]).css('display', '');
			$("#location_filter_"+edir_locations[i]).val(0);
		}

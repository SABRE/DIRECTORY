	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($validate_account && $validate_contact && $validate_article && $validate_discount) {
			...
			if ($payment_method == "checkout") {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=article&item_level=".$level."&checkout=true&payment_method=checkout&item_id=".$article->getNumber("id");
				} else {
				...
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=article&item_level=".$level."&checkout=false&payment_method=invoice&item_id=".$article->getNumber("id");
				} else {
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/invoice.php";
				}
			} else {
				if ($hasPackage){
                    $redirectURL = DEFAULT_URL."/".MEMBERS_ALIAS."/signup/packages.php?item_type=article&item_level=".$level."&checkout=false&payment_method=".$payment_method."&item_id=".$article->getNumber("id");
				} else {

<?
	...
	# PINTEREST PLUGIN
	#
	# Turn on this flag if you want to show the Pinterest button on detail pages.
    define("PINTEREST_PLUGIN", "on");
	...
	# CATEGORY CONSTANTS
	# ----------------------------------------------------------------------------------------------------
	define("MAX_CATEGORY_ALLOWED",              10); // Max 5 relationship (All modules except Listing)
	define("LISTING_MAX_CATEGORY_ALLOWED",      10); // Put the limit that you want of categories to listings
	...
	if (file_exists(EDIRECTORY_ROOT."/conf/constants_".EDIR_THEME.".inc.php")) {
        ...
        define("FEATURED_LISTING_MAXITEMS_SPECIAL", 4);
        ...
        # NEW STYLE FOR "REMOVE FROM FAVORITES"
        define("THEME_REMOVE_FAVORITES_DIV", false);
    }
    ...
    define("CACHE_FULL_FILE_NAME", md5($_SERVER["HTTP_HOST"].(($_SERVER["REQUEST_URI"] == "/") ? "/index.php" : $_SERVER["REQUEST_URI"])).".html");

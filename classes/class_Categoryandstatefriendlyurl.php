<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /classes/class_Categoryandstatefriendlyurl.php
	# ----------------------------------------------------------------------------------------------------

	/**
	 * <code>
	 *  $categoryandstatefriendlyurlObj = new Categoryandstatefriendlyurl($id);
	 * <code>
         * @package Classes
	 * @name Categoryandstatefriendlyurl
	 * @method Categoryandstatefriendlyurl
	 * @method Save
	 * @method Delete
         * @method Edit
	 * @access Public
	 */
class Categoryandstatefriendlyurl extends Handle 
{
    ...
    /**Function is created on 16-09-2013*/
    function Categoryandstatefriendlyurl($var='', $domain_id = false) 
    {
        if (is_numeric($var) && ($var)) 
        {
            ...
            unset($dbMain);
            $sql = "SELECT * FROM category_state_friendlyurl WHERE id = $var";
    ...
    function makeFromRow($row='') 
    {
            ...
            $this->state_id = ($row["state_id"]) ? $row["state_id"]: ($this->state_id ? $this->state_id : 0);
            $this->friendly_url = ($row["friendly_url"]) ? $row["friendly_url"]	: ($this->friendly_url ? $this->friendly_url : "");
            $this->status = ($row["status"]) ? $row["status"] : ($this->status ? $this->status : 0);
            $this->created = ($row["created"]) ? $row["created"] : "";
            $this->modified = ($row["modified"]) ? $row["modified"] : "";
    }

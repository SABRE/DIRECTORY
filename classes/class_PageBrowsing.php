<?
    ...
    class pageBrowsing extends Handle   
    {
            ...
            function retrievePage($return_type = "object", $aux_total_items = false, $override = false, $loc = false, $module_category = false, $module_category_id=0, $show_country = 0, $using_location2 = 0, $using_location3 = 1,&$added_info=false,&$choose_banner_to_approve=false) 
            {
                ...
                if ($this->table == "profile_review") {
                    ...
                    if ($mysqlversion > 3) {
                        ...
                        if ($this->where["listing"]){
                            $sql .= "id,title as title,'name' as name,friendly_url as friendly_url,thumb_id,level as level,account_id,address as address,address2 as address2,listingtemplate_id as listingtemplate_id,claim_disable as claim_disable,video_snippet as video_snippet,phone as phone,zip_code as zip_code,'map_zoom' as map_zoom, description as description, fax as fax,display_url as display_url,url as url,email as email,attachment_file as attachment_file,attachment_caption as attachment_caption,long_description as long_description,hours_work as hours_work,locations as locations,description as description,'publication_date' as publication_date,'author' as author,'author_url' as author_url, 'abstract' as abstract,'summarydesc' as summarydesc, phone as phone, email as email,'classified_price' as classified_price,url as url,'aListing' as module FROM Listing WHERE id IN (".$this->where['listing'].")";
                    ...
                    */
                    ...
                    if ($return_type == "object") {
                        ...
                        if($this->table=="Banner" && $override==true)
                        {
                             
                            $results = $result;
                            unset($result);
                            foreach($results as $banner_cat)
                            {
                                $location_name = "";
                                $connector = "";
                                ...
                                $process_array = false;
                                if($loc!="" && $module_category && $module_category == $banner_cat['section'])
                                {
                                    if(($loc == $location_link||$loc == $location_link_country||$loc == $location_link_region) && $module_category_id== $banner_cat['category_id'])
                                    {
                                        $process_array = true;
                                ...
                                else if($module_category && $module_category == $banner_cat['section'])
                                {
                                    if($module_category_id == $banner_cat['category_id'])
                                    {
                                        $process_array = true;
                                ...
                                if($this->letter)
                                {
                                    if(strtolower($this->letter)==strtolower($category_name[0]))
                                        $process_array = true;
                                    else
                                        $process_array = false;
                                }
                                ...
                                if(isset(${"categoy_banner_".$banner_cat['section']}))
                                {
                                    $result[$banner_cat['section']][0] = ${"categoy_banner_".$banner_cat['section']};
                                    $result[$banner_cat['section']][1]	= count(${"categoy_banner_".$banner_cat['section']});
                                    if ($this->limit){
                                            $result[$banner_cat['section']][2]	= ceil(count(${"categoy_banner_".$banner_cat['section']})/$this->limit);
                                            $result[$banner_cat['section']][3]	= ($this->screen >= ceil(count(${"categoy_banner_".$banner_cat['section']})/$this->limit)) ? ceil(count(${"categoy_banner_".$banner_cat['section']})/$this->limit) : ($this->screen+1);
                                            $result[$banner_cat['section']][4]	= ($this->screen <= 1) ? 1 : ($this->screen-1);
                                            $result[$banner_cat['section']][5]	= $this->calculatePageJump(count(${"categoy_banner_".$banner_cat['section']}));
                                    }
                                }
                            }
                        }
		...
		function calculatePageJump($override=0) {
			if($override>0)
				$amount = $override;
			else
		...
		*/
		function getPagination($page_num, $limit, $aux_page_url = false, $force_total = false){
		...
		function getPagesDropDown ($getData, $pagingUrl, $screen = 1, $defaultText = "Go to page: ", $defaultOnChange = "this.form.submit();", $id = "", $array_params = false, $override=false, $information_array = false) {
			...
			if (string_strpos($pagingUrl, "/".SOCIALNETWORK_FEATURE_NAME) !== false || string_strpos($pagingUrl, "".MEMBERS_ALIAS."/account") !== false) {
				$profile = true;

				$defaultOnChange = "changePageProfile(\"".$pagingUrl."\", this.value)";
			}
			...
			if($override)
			{
				$increment = ($information_array[5] <= 0) ? 1 : $information_array[5];
				$increment2 = $information_array[5];
				$i = 1;
				while ($i <= $information_array[2]) {
					if ($screen == $i) {
						$pagesDropDown .= "<option value=\"$i\" selected=\"selected\">".$i."</option>\n";
					} elseif (($screen != 1) && ($i > $screen) && (($i-$information_array[5]) < $screen)) {
					...
					if ($i == 1) {
						...
						if (($i < $information_array[2]) && (($i+$increment2) > $this->pages)) {
							$i += ($information_array[2])-$i;
						} else {
							$i += $increment2;
						}
					}
		...
		function getPaginationForListing($page_num, $limit, $aux_page_url = false, $force_total = false){
			...
			$aux_pagination_code = $pagesObj->getCodeOfPaginationForListing($array_pages, $aux_page_url, false, $force_total);
			return $aux_pagination_code;

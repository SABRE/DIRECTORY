		...
		function Delete($updFullText = false) {
			...
			if ($this->id) {
				...
				while ($row = mysql_fetch_assoc($resDomain)) {
					...
					$result = $dbObj->query($sql);
					//Banner
					$sql = "UPDATE Banner SET feature_onlocation3=0 WHERE feature_onlocation3 = $this->id";
					$result = $dbObj->query($sql);
				}
				unset($rowDomain);
...
if($this->latitude!=0 && $this->longitude!=0)
            ...
            $dbObj = db_getDBObject(DEFAULT_DB, true);
            $sql = "UPDATE Location_3 SET latitude = '$this->latitude', longitude = '$this->longitude' WHERE id = '$this->id'";
            $dbObj->query($sql);
        }

...
//			$dbMain->close();
        ...
        function getReviewByItemID(){
            ...
            $db = db_getDBObject();
            $sql = "select * from Review where item_type = ".db_formatString($this->item_type)." AND item_id = ".db_formatNumber($this->item_id);
            $result = $db->query($sql);
        ...
        function GetTotalReviewsByItemID(){
            $db = db_getDBObject();
            $sql = "SELECT item_type, item_id FROM Review WHERE item_type = ".db_formatString($this->item_type)." AND item_id = ".db_formatNumber($this->item_id);

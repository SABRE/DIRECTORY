...
class ListingLevel {
    ...
    function getBacklink($value) {
        ...
        if (isset($backlinkArray[$value]))
            return $backlinkArray[$value];
        else
            return $backlinkArray[$this->default];
    }
    ...
    function getHasPromotion($value) {
        ...
        if (isset($haspromotionArray[$value]))
            return $haspromotionArray[$value];
        else
            return $haspromotionArray[$this->default];
    }
    ...
    function getHasSms($value) {
        ...
        if (isset($hassmsArray[$value]))
            return $hassmsArray[$value];
        else
            return $hassmsArray[$this->default];
    }

    function getHasCall($value) {
        $hascallArray = $this->union($this->value, $this->has_call);
        if (isset($hascallArray[$value]))
            return $hascallArray[$value];
        else
            return $hascallArray[$this->default];
    }
    ...
    function getContent($value) {
        ...
        if (isset($contentArray[$value]))
            return $contentArray[$value];
        else
            return $contentArray[$this->default];
    }
    ...
    function updateValues($name, $active, $has_promotion, $has_review, $has_sms, $has_call, $backlink, $levelValue) {

        $dbMain = db_getDBObject(DEFAULT_DB, true);
        $dbObj = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbMain);

        $sql = "UPDATE ListingLevel SET name = '" . $name . "', active = '" . $active . "', has_promotion = '" . $has_promotion . "', has_review = '" . $has_review . "', has_sms = '" . $has_sms . "', has_call = '" . $has_call . "', backlink = '" . $backlink . "' WHERE value = " . $levelValue . " AND theme = '" . (EDIR_THEME ? EDIR_THEME : "default") . "'";
        $dbObj->query($sql);
    }

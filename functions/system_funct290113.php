...
//		$total_year = $total_payment_year + $total_invoice_year;
	...
	function system_advancedSearch_getCategories($type = "listing", $filter = false, $selected_category_id = 0, $show_type = "all", $main_id=0, $template_id=0){
		...
		if($show_type == "sub" && $main_id)
		    $sql_categories = "SELECT id, title FROM $table WHERE id=".$main_id." AND category_id = 0 AND title <> '' AND enabled = 'y' ORDER BY title LIMIT ".MAX_SHOW_ALL_CATEGORIES;
		else
								...
								if(count($xml_subcategories->item) > 0) {
									for($j=0;$j<count($xml_subcategories->item);$j++){
										...
										if (count($subcategory > 0)) {
											if($show_type == "all"||$show_type == "sub")
											{
		...
		if($filter)
		{
			if($show_type == "sub")	
			{
				if(count($valueArray)==1 && $valueArray[0]=="")
					return "empty";
				$categoryFilterSubDD = html_selectBoxCat("category_sub_filter_id", $nameArray, $valueArray, ($selected_category_id!=0?$selected_category_id:""), "", "", system_showText(LANG_SEARCH_LABELCBSUBCATEGORY), $type);	

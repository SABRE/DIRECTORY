<?
	...
	function zipproximity_getWhereZipCodeProximity($zip, $dist, &$whereZipCodeProximity, &$order_by_zipcode_score,$latitudeIPhone=false,$longitudeIPhone=false, $promotion = false, $locationBased = false) {
		...
		if (zipproximity_getZip5($zip, $zip5)  || ($latitudeIPhone && $longitudeIPhone)) {
			if (zipproximity_validateZip5($zip5)  || ($latitudeIPhone && $longitudeIPhone)) {
				if (zipproximity_getZip5Fields($zip5, $latitude, $longitude) || ($latitudeIPhone && $longitudeIPhone)) {
					...
					{
						if (ZIPCODE_UNIT == "mile") {
							$order_by_zipcode_score = "SQRT(POW((69.1 * (".$latitude." - $latitudeField)), 2) + POW((53.0 * (".$longitude." - $longitudeField)), 2)) AS location_score";
						} elseif (ZIPCODE_UNIT == "km") {
							$order_by_zipcode_score = "SQRT(POW((69.1 * (".$latitude." - $latitudeField)), 2) + POW((53.0 * (".$longitude." - $longitudeField)), 2)) * 1.609344 AS location_score";
						}
					}
					return true;
				}
	...
	function zipproximity_getLocationDistanceLabel( $lat, $long, $item, $id, &$distance_label, $have_data = false, $data = "") {
		$distance_label = "";
		if ($lat && $long && $item && $id && ZIPCODE_PROXIMITY == "on") {
						if ($have_data) {
							$item_latitude = $data["latitude"];
							$item_longitude = $data["longitude"];
							if (ZIPCODE_UNIT == "mile") {
								$distance_label = (round(sqrt(pow((69.1 * ($lat - $item_latitude)), 2) + pow((53.0 * ($long - $item_longitude)), 2)), 2))." ".ZIPCODE_UNIT_LABEL_PLURAL;
							} elseif (ZIPCODE_UNIT == "km") {
								$distance_label = (round(sqrt(pow((69.1 * ($lat - $item_latitude)), 2) + pow((53.0 * ($long - $item_longitude)), 2)) * 1.609344, 2))." ".ZIPCODE_UNIT_LABEL_PLURAL;
							}
						} else {
							$dbObjMain = db_getDBObject(DEFAULT_DB, true);
							$dbObj = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbObjMain);
	...
	/*function zipproximity_getWhereLocationProximity( $lat, $long, $dist, &$whereLocationProximity, &$order_by_zipcode_score,$latitudeIPhone=false,$longitudeIPhone=false, $promotion = false) {

	...
	*/
	...
	function validate_form($form, $array, &$error, $bannerErrorSize = 0) {
        ...
        elseif ($form == "blogseocenter") {
			...
			if ($type==4 && strpos($_SERVER["PHP_SELF"],"rder_banner")==false) 
			{
				if($category_id==0)
				$errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_CATEGORY_IS_REQUIRED);
				if ($approve_feature=="y"||$approve_feature=="P") 
				{
					if(isset($location_1) && $location_1=="") $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_LOC1_IS_REQUIRED);
					if(isset($location_2) && ($location_2=="" && $new_location2_field=="")) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_LOC2_IS_REQUIRED);
					if(isset($location_3) && ($location_3=="" && $new_location3_field=="")) $errors[] = "&#149;&nbsp;".system_showText(LANG_MSG_LOC3_IS_REQUIRED);
				}
			}
    ...
    function validate_signupPages($isforeignAcc = false) {
        ...
        if ($isforeignAcc == "y") {
            return true;
        } elseif (string_strpos($page, MEMBERS_ALIAS."/account/account.php") !== false || string_strpos($page, SITEMGR_ALIAS."/account/account.php") !== false || string_strpos($page, "profile/add.php") !== false || string_strpos($pageClaim, ALIAS_CLAIM_URL_DIVISOR) !== false || string_strpos($page, "profile/edit.php") !== false || string_strpos($page, "order_") !== false) {
            return false;
        } else {
            return true;
        }

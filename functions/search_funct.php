<?
		...
		if ($search_for["zip"]) {
			...
			if (ZIPCODE_PROXIMITY == "on" && $search_for["dist"]) {
				if (zipproximity_getWhereZipCodeProximity($search_for["zip"], $search_for["dist"], $whereZipCodeProximity, $order_by_zipcode_score)) {
					...
					$where_clause[] = ($section == "listing_results"  || FORCE_SECOND ? "Listing_Summary" : "Listing").".zip_code LIKE '".$search_for["zip"]."%'";
				}
			} else {
				$where_clause[] = ($section == "listing_results" || FORCE_SECOND ? "Listing_Summary" : "Listing").".zip_code LIKE '".$search_for["zip"]."%'";
			}
        ...
        if ($search_for["zip"]) {
            ...
            if (ZIPCODE_PROXIMITY == "on" && $search_for["dist"]) {
                if (zipproximity_getWhereZipCodeProximity($search_for["zip"], $search_for["dist"], $whereZipCodeProximity, $order_by_zipcode_score, false, false, true)) {
                    ...
                    if (!$search_for["keyword"] || $section == "count") {
                        $where_clause[]     = "Promotion.listing_zipcode LIKE '".$search_for["zip"]."%'";
                    } else {
                        $having_clause[]    = "Promotion.listing_zipcode LIKE '".$search_for["zip"]."%'";
                    }
                }
            } else {
                if (!$search_for["keyword"] || $section == "count") {
		...
		if ($search_for["zip"]) {
			...
			if (ZIPCODE_PROXIMITY == "on" && $search_for["dist"]) {
				if (zipproximity_getWhereZipCodeProximity($search_for["zip"], $search_for["dist"], $whereZipCodeProximity, $order_by_zipcode_score)) {
					...
					$where_clause[] = "Event.zip_code LIKE '".$search_for["zip"]."%'";
				}
			} else {
				$where_clause[] = "Event.zip_code LIKE '".$search_for["zip"]."%'";
			}
		...
		if ($search_for["zip"]) {
			...
			if (ZIPCODE_PROXIMITY == "on" && $search_for["dist"]) {
				if (zipproximity_getWhereZipCodeProximity($search_for["zip"], $search_for["dist"], $whereZipCodeProximity, $order_by_zipcode_score)) {
					...
					$where_clause[] = "Classified.zip_code LIKE '".$search_for["zip"]."%'";
				}
			} else {
				$where_clause[] = "Classified.zip_code LIKE '".$search_for["zip"]."%'";
			}
		...
		if ($search_for["zip"]) {
			...
			if (ZIPCODE_PROXIMITY == "on" && $search_for["dist"]) {
				if (zipproximity_getWhereZipCodeProximity($search_for["zip"], $search_for["dist"], $whereZipCodeProximity, $order_by_zipcode_score)) {
					...
					$where_clause[] = "Article.zip_code LIKE '".$search_for["zip"]."%'";
				}
			} else {
				$where_clause[] = "Article.zip_code LIKE '".$search_for["zip"]."%'";
			}
	...
	function search_getSQLFullTextSearch($searchfor, $fields, $order_by_fieldname, &$order_by_score, $force_specific_search="", &$order_by_score2, $order_by_fieldname2 = "") {
		...
		if ($force_specific_search == "exactmatch") {
			...
			foreach($words_array as $each_searchfor)
			{
				if(string_strlen($each_searchfor) < (int)FT_MIN_WORD_LEN)
					$counter++;
			}
			if($counter == count($words_array))
			{
				unset($searchfor_aux_array);
				foreach ($words_array as $each_searchfor) {
					$searchfor_aux = $each_searchfor;
    ...
    /*Fucntions is created to links for Sorting*/
    /*function search_getSortingLinks ($getData, $pagingUrl, $orderBy, $defaultText = "Order by: ", $defaultOnChange = "this.form.submit();", $array_params = false, $deal = false, $blog = false) {
		...
		//$option_Link = $pagingUrl.$url_search_params.'/orderby/'.$option_value;
		//$orderbyDropDown .= "<li><a href='".$option_Link."'>A - Z</a></li>";
		$orderbyDropDown .= "<li><a href='javascript:void(0)' onclick='changePageOrder(\"".$pagingUrl."\",\"".$option_value."\",\"".$url_search_params."\")'>A - Z</a></li>";
		 
		$option_value = "rating";
		//$option_Link = $pagingUrl.$url_search_params.'/orderby/'.$option_value;
		//$orderbyDropDown .= "<li><a href='".$option_Link."'>Rating</a></li>";
		$orderbyDropDown .= "<li><a href='javascript:void(0)' onclick='changePageOrder(\"".$pagingUrl."\",\"".$option_value."\",\"".$url_search_params."\")'>Rating</a></li>";
		  
		$option_value = "popular";
		//$option_Link = $pagingUrl.$url_search_params.'/orderby/'.$option_value;
		//$orderbyDropDown .= "<li><a href='".$option_Link."'>Popularity</a></li>";
		$orderbyDropDown .= "<li><a href='javascript:void(0)' onclick='changePageOrder(\"".$pagingUrl."\",\"".$option_value."\",\"".$url_search_params."\")'>Popularity</a></li>";
		$orderbyDropDown .= "</form>";
		return $orderbyDropDown;
        ...
        /*Function regenrate on 20-09-2013 for friendly url*/
        function search_getSortingLinks($getData,$pagingUrl,$parts,$friendlyurl,$url_search_params) 
        {
           if($getData['orderby']){
                    unset($getData['orderby']);
            }
            if($getData['screen']){
                    unset($getData['screen']);
            }
            if($friendlyurl)
            {
                $url_search_params = '';
                ...
                if(!empty($extraUrl)){
                    $pagingUrl = rtrim($pagingUrl,'/').$extraUrl;
                }
	...
	function search_pagePrevNextLink($tableName = false, $listingId = null , $flag = false){
		...
		$returnUrl = '';
                if($tableName != 'Promotion'){
                    if(($tableName) && ($listingId > 0) && ($flag)){ 
                            if($flag == 'prev'){
                                    $sql = "SELECT $tableName.friendly_url as url FROM $tableName WHERE $tableName.id < ".$listingId." AND  $tableName.level = 10 order by $tableName.id DESC LIMIT 1";
                            }elseif($flag == 'next'){
                                    $sql = "SELECT $tableName.friendly_url as url FROM $tableName WHERE $tableName.id > ".$listingId." AND  $tableName.level = 10 order by $tableName.id ASC LIMIT 1";
                            }
                            $result = $db->query($sql);
                        $returnUrlArray = mysql_fetch_assoc($result);
                        $returnUrl = $returnUrlArray['url'];
                    }
                }else{
                    if(($tableName) && ($listingId > 0) && ($flag)){ 
                            if($flag == 'prev'){
                                    $sql = "SELECT $tableName.friendly_url as url FROM $tableName WHERE $tableName.id < ".$listingId." order by $tableName.id DESC LIMIT 1";
                            }elseif($flag == 'next'){
                                    $sql = "SELECT $tableName.friendly_url as url FROM $tableName WHERE $tableName.id > ".$listingId." order by $tableName.id ASC LIMIT 1";
                            }
                            $result = $db->query($sql);
                        $returnUrlArray = mysql_fetch_assoc($result);
                        $returnUrl = $returnUrlArray['url'];

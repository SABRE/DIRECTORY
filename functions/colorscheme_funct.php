		...
		if (!file_exists($dynamicCSSPath) || (int)filesize($dynamicCSSPath) <= 0 || $scheme_updatefile == "on"){ //update css file
			...
			foreach ($constReplace as $const) {
				...
				if ($const == "SCHEME_BACKGROUNDIMAGE") { //background image
					...
					if (file_exists(EDIRECTORY_ROOT."/custom/domain_".SELECTED_DOMAIN_ID."/theme_images/".EDIR_SCHEME."_active.png") && EDIR_SCHEME != "golfcourse" && EDIR_SCHEME != "companies" && EDIR_SCHEME != "dentist" && EDIR_SCHEME != "magazine" && EDIR_SCHEME != "environment" && EDIR_SCHEME != "edirectorycompact" && EDIR_SCHEME != "edirectory7" && EDIR_SCHEME != "happytimes"){ //replace with the new menu active image
				...
				if ($newValue) {
					...
					if (EDIR_SCHEME == "spa"){ //exception to Spa scheme
                        ...
                        $regexPattern = "/\/\*Marker Spa Exception5\*\/.*\/\*Marker Spa Exception5\*\//s";
                        $replace_spa = "";
                        $phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
                        
                        $regexPattern = "/\/\*Marker MembersSpa\*\/.*\/\*Marker MembersSpa\*\//s";
                        $replace_spa = "";
                        $phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
                        
                        $regexPattern = "/\/\*Marker Spa\*\/.*\/\*Marker Spa\*\//s";
                        $replace_spa = "";
                        $phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
                    ...
                    if (EDIR_SCHEME != "edirectorycompact"){ //remove eDirectory Compact exception
						$regexPattern = "/\/\*Marker eDirCompact\*\/.*\/\*Marker eDirCompact\*\//s";
						$replace_spa = "";
						$phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
					}
					...
					if (EDIR_SCHEME != "environment"){ //remove Environment exception
                        ...
                        $regexPattern = "/\/\*Marker InvoiceEnvironment\*\/.*\/\*Marker InvoiceEnvironment\*\//s";
						$replace_spa = "";
						$phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
                        
                        $regexPattern = "/\/\*Marker MembersEnviroment\*\/.*\/\*Marker MembersEnviroment\*\//s";
						$replace_spa = "";
						$phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
					}
                    
                    if (EDIR_SCHEME != "medical"){ //remove Medical exception
						$regexPattern = "/\/\*Marker TableMedical\*\/.*\/\*Marker TableMedical\*\//s";
						$replace_spa = "";
						$phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
					}
					
					if (EDIR_SCHEME == "happytimes" || EDIR_SCHEME == "environment"){ //exception to Happy Times scheme
						$regexPattern = "/\/\*Marker HappyTimes Exception\*\/.*\/\*Marker HappyTimes Exception\*\//s";
                        if (EDIR_SCHEME == "happytimes"){
                            $replace_spa = ".login button
					...
					if (EDIR_SCHEME == "buyersguide"){
						$regexPattern = "/\/\*Marker TB Exception\*\/.*\/\*Marker TB Exception\*\//s";
						$replace_spa = "#TB_window, .modal-content
										{ color:#".SCHEME_COLORMAINCONTENT."; } ";
						$phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
					} elseif (EDIR_SCHEME == "restaurant"){
						$regexPattern = "/\/\*Marker TB Exception\*\/.*\/\*Marker TB Exception\*\//s";
						$replace_spa = "#TB_window, .modal-content
										{ color:#".SCHEME_COLORCONTENTBACKGROUND."; } ";
						$phpContent = preg_replace($regexPattern, $replace_spa, $phpContent);
					} elseif (EDIR_SCHEME == "cityguide" || EDIR_SCHEME == "cityguideyellow" || EDIR_SCHEME == "edirectoryclassic" || EDIR_SCHEME == "edirectorycompact" || EDIR_SCHEME == "companies" || EDIR_SCHEME == "financial" || EDIR_SCHEME == "happytimes" || EDIR_SCHEME == "hotel" || EDIR_SCHEME == "sports"){
                        $regexPattern = "/\/\*Marker TB Exception\*\/.*\/\*Marker TB Exception\*\//s";
						$replace_spa = "#TB_window, .modal-content

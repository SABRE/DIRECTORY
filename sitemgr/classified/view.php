			...
			<? } else { ?>
                ...
                <ul class="list-view columnListView">
					...
					<? if ((GOOGLE_MAPS_ENABLED == "on") && ($mapObj->getString("value") == "on") && (($classified->getString("address")) || ($classified->getNumber("location_3")) || ($classified->getNumber("location_4")  || ($classified->getString("latitude") && $classified->getString("longitude"))))) { ?>
						<li><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=CLASSIFIED_FEATURE_FOLDER;?>/maptuning.php?id=<?=$classified->getNumber("id")?>&screen=<?=$screen?>&letter=<?=$letter?><?=(($url_search_params) ? "&$url_search_params" : "")?>" class="link-view"><?=string_ucwords(system_showText(LANG_SITEMGR_MAPTUNING))?></a></li>
					<? } ?>
